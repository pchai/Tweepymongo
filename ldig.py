...
class ldig(object):
    def __init__(self, model_dir):
        ...
        self.myparam = numpy.load(self.param)
    ...
    def detect(self, tweet, language):

        return likelihood(self.myparam , self.labels, self.trie, tweet, language)
...
def likelihood(param, labels, trie, tweet, language):

    timestamp1 = datetime.datetime(2013, 1, 25, 18, 0)
    ...
    if tweet.get("id_str") is not None and tweet.get("id_str") is not None:
        input_text = "\t[" + tweet.get("id_str") + "]\t[" + tweet.get("text")+ "]\n".encode('utf-8')
        label, text, org_text = normalize_text(input_text)
        ...
        if y[predict_k] < 0.6:
            predict_lang = ""
        if predict_lang == language:
            return True
            print "[" + tweet.get("created_at") + "]\t" + tweet.get("text")
