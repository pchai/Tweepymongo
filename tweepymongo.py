...
def get_parser():
    ...
    parser.add_option("-p", "--port", dest="port", default="27011", help="mongodb port")
    parser.add_option("-c", "--collection", dest="collection", default="Tweets", help="mongodb collection name")
    parser.add_option("-u", "--username", dest="username", default="", help="username for mongodb")
    parser.add_option("-w", "--passwd", dest="passwd", default="", help="passwd for mongodb")
    parser.add_option("-o", "--oauth", dest="oauthfile", default="oauth.json", help="file with oauth options")
    parser.add_option("-q", "--wordfile", dest="querywords", default="query.txt", help="file with query words")
...
class MongoDBCoordinator:
    ...
    def __init__(self, host, database, port, collection, username, passwd, query):
        self.host = host
...
if __name__ == "__main__":
    ...
    mongo = MongoDBCoordinator(options.server, options.database, options.port, options.collection,
                                            options.username, options.passwd, query)
